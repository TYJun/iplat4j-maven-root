...
//				});
				...
				$("#START").on("click",function(e){
					if (datagrid == null) {
						...
						EiCommunicator.send("IMKP01", "openInfo",
								eiInfo, {
				...
				$("#END").on("click",function(e){
					if (datagrid == null) {
						...
						EiCommunicator.send("IMKP01", "endInfo",
								eiInfo, {
							onSuccess : function(ei) {
								IPLAT.alert(ei.getMsg());
								window.parent["resultGrid"].dataSource.query(1);
...
function popData() {
	var url = IPLATUI.CONTEXT_PATH + "/web/IMKP02";
	var popData = $("#popData");
...
function popDataUpdate(cardids) {
	var url = IPLATUI.CONTEXT_PATH + "/web/IMKP0201?initLoad&id="+cardids;
	var popData = $("#popData");
