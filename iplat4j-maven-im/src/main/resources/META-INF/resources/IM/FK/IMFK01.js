		...
		"result" : {
			loadComplete : function(grid) {
				$("#SENDCARD").on("click", function(e) {
					...
					if(row == undefined){
						NotificationUtil("请选择一个巡查对象进行发卡", "error");
						return;
					}
					var checkedSpotNum = row.spotCode;
					var id = row.spotId;
					sendCard(id);
					if (resultCode != "1") {
					...
					EiCommunicator.send("IMFK01", "sendCard", info, {
				...
				$("#QRCODE").on("click", function(e) {
					var row = resultGrid.getCheckedRows()[0];
					if(row == undefined){
						NotificationUtil("请选择一个巡查对象生成二维码", "error");
						return;
					}
					var id = row.id;
				...
				function popData(id,spotName) {
					var url = IPLATUI.CONTEXT_PATH + "/web/IMFK0101?&id="+id+"&spotName="+spotName;
					var popData = $("#popData");
...
function getSpotName(cardCode) {
	...
	EiCommunicator.send("IMFK01", "querySpotName", info, {
		onSuccess : function(ei) {
