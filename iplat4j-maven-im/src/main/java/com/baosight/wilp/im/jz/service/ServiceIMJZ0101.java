...
import com.baosight.wilp.common.web.MobileServiceDispatcher;
...
public class ServiceIMJZ0101 extends ServiceBase {

    protected static final Logger logger = LoggerFactory.getLogger(ServiceIMJZ0101.class);
	...
	public EiInfo initLoad(EiInfo inInfo) {
		...
		if (operType.equals("add")) {
			...
			//查询基准
			List<Map<String, Object>> schemeInfo = dao.query("IMJZ01.queryScheme", param);
			
			//查询周期
			List<Map<String, Object>> cycle = dao.query("IMJZ01.queryCycle", id);
			//查询项目
			List<Map<String, Object>> item = dao.query("IMJZ01.queryItem", id);
			item.forEach(i -> {
			...
			//查询执行人员
			List<Map<String, Object>> exman = dao.query("IMJZ01.queryExman", id);
			item.forEach(m -> {
			...
			//查询设备信息
			List<Map<String, Object>> device = dao.query("IMJZ01.queryDevice", objectID);
    ...
    public EiInfo query(EiInfo inInfo) {
        ...
        if (operType.equals("add")) {
            ...
            String machineId = (String)map.get("machineID");
            //查询设备信息
            List<Map<String, Object>> device = dao.query("IMJZ01.queryDevice", machineId);
...
//	}
	...
	public EiInfo insertScheme(EiInfo eiInfo) {
        ...
        logger.info("安全巡检基准【IMJZ0101】操作类型operType==", operType);
        /*如果是更新操作，则先删除原来数据，再重新添加基准及基准周期、关联设备、巡查项目、执行人列表等*/
        if ("update".equals(operType)) {
            dao.delete("IMJZ01.deleteOneScheme", viewSchemeID);
            dao.delete("IMJZ01.deleteCycle", viewSchemeID);// 删除执行周期
            dao.delete("IMJZ01.deleteItem", viewSchemeID);// 删除基准项目
            dao.delete("IMJZ01.deleteExman", viewSchemeID);// 删除执行人
        }
        ...
        deviceList.forEach(device -> {
            String schemeID;
            String schemeCode;
            String objId = (String)device.get("objId");
            String objName = (String)device.get("objName");
            ...
            itemList.forEach(item -> {
                ...
                dao.insert("IMJZ01.insertItem", insertItem);
            });
            ...
            cycleList.forEach(cycle -> {
                ...
                dao.insert("IMJZ01.insertCycle", cycle);
            });
            ...
            exmanList.forEach(exman -> {
                ...
                dao.insert("IMJZ01.insertExman", insertExman);
            });
    ...
    public EiInfo device(EiInfo eiInfo) {
        ...
        List<Map<String, String>> query = dao.query("IMJZ0101.queryDevice", id);
