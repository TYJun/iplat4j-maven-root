...
import com.baosight.wilp.im.jz.service.ServiceIMJZApp;
import com.baosight.wilp.im.zh.domain.IMZH0101;
...
public class ServiceIMZH0101 extends ServiceBase{
	...
	public EiInfo initLoad(EiInfo info) {
		...
		//查询基准
		List<Map<String, Object>> schemeInfo = dao.query("IMZH01.query", param);
		
		
		//查询项目,循环修改字段名称
			List<Map<String, Object>> item = dao.query("IMZH01.queryItem", param);
			int itemCount=dao.count("IMZH01.queryItemCount", param);
			item.forEach(m -> {
		...
		//查询执行人员,循环修改字段名称
		List<Map<String, Object>> exman = dao.query("IMZH01.queryExman", param);
		exman.forEach(m -> {
		...
		//查询图片信息
        List<IMZH0101> queryPic = dao.query("IMZH01.queryPic", taskID);
		
		
		
		if(null != queryPic) {
            for (IMZH0101 dizh0101 : queryPic) {
        ...
        addBlock2.addBlockMeta(new IMZH0101().eiMetadata);
