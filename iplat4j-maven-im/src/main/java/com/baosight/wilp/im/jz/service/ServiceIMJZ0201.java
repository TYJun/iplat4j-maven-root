 ...
 * <desc>    // 描述修改内容
 */
public class ServiceIMJZ0201 extends ServiceBase{
    ...
    public EiInfo insr(EiInfo inInfo) {
        ...
        //保存设备包
        dao.insert("IMJZ0201.insertPack", packMap);
        Map mapDetail = new HashMap();
        //循环保存设备包内容
        for (Map<String, String> map : data) {
            mapDetail.put("packet_id", randomUUID.toString());
            mapDetail.put("device_id", UUID.randomUUID().toString());
            ...
            dao.insert("IMJZ0201.insertPackDetail", mapDetail);
        }
        inInfo.setMsg("保存成功");
        inInfo.setStatus(1);
        return inInfo;
    ...
    public static String dangerCode() {
        ...
        //查询当天是否生成过编号
        List query = dao1.query("IMJZ0201.dangerCodeCount", "IM"+substring);
        String count = query.get(0).toString();
        Integer valueOf = Integer.valueOf(count);
        Integer par = valueOf + 1;
        String string = par.toString();
        String code = "IM"+ substring + "00" + string;
        return code;
    }
