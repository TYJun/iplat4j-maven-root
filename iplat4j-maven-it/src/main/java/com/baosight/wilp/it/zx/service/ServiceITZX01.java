 ...
 * 第三段
 * @author zhaowei
 * @date 2022年07月28日 17:10
 * @version V1.0
 */
public class ServiceITZX01 extends ServiceBase {
	/*
	 * 第一段
	 * 独立任务执行初始化方法，调用独立任务执行查询方法.
	 * 第二段
	...
	public EiInfo query(EiInfo info) {
		...
		/*
		 * 4.进行数据库查询操作
		 * 通过查询参数集合以及分页参数查询独立任务执行信息
		 * 通过账套（dataGroupCode）、登记状态（status）、任务单号（taskNo）、服务分类（parentId）、
		 * 需求科室（reqDeptName）、服务科室（serveDeptName）、创建时间起（recCreateTimeS）、创建时间止（recCreateTimeE）
		 */
		List<Map<String,Object>> queryitTaskInfoList = dao.query("ITZX01.queryItTaskInfo", paramsMap, (Integer) pageMap.get("offset"), (Integer) pageMap.get("limit"));
		int count = dao.count("ITZX01.queryItTaskInfo", paramsMap);
