$(function () {
    ...
    $("#SUBMIT").unbind('click').on('click', function() {
        ...
        if(!validate(manNo,manName,sex,deptNum,employmentNature,age,phone,identityCard,maritalStatus,educationBackground,permanentResidence,hiredate)){
            return;
        }
        ...
        EiCommunicator.send("DMRZ0101", "insert", eiInfo, {
...
function validate(manNo,manName,sex,deptNum,employmentNature,age,phone,identityCard,maritalStatus,educationBackground,permanentResidence,hiredate){
    if(isEmpty(manNo)){
    ...
    if(isEmpty(maritalStatus)){
        NotificationUtil("婚姻状况不能为空", "error");
        return false;
    }
    if(isEmpty(educationBackground)){
        NotificationUtil("学历不能为空", "error");
        return false;
    }
    if(isEmpty(permanentResidence)){
        NotificationUtil("户口所在地不能为空", "error");
        return false;
    }
    if(isEmpty(hiredate)){
