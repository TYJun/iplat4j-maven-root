...
/* 		var deposit = IPLAT.EFInput.value($("#deposit"));
		...
		//参数校验
		if(!validate(name,departmentName,employmentNature,workTime,age)){
			return;
		}
		if(type === "edit"){
			eiInfo.set("id",id);
...
//参数校验
function validate(name,departmentName,employmentNature,workTime,age){

	if(isEmpty(name)){
		NotificationUtil("入住人姓名不能为空", "error");
		return false;
	}

	if(isEmpty(departmentName)){
		NotificationUtil("部门科室名称不能为空", "error");
		return false;
	}
	...
	if(isEmpty(employmentNature)){
		NotificationUtil("入住人职工属性不能为空", "error");
		return false;
	} 
/*	if(isEmpty(phone)){
		NotificationUtil("入住人手机不能为空", "error");
		return false;
	} */
...
$("#name").kendoAutoComplete({
	dataSource: dataSourceConfig("/service/DMHM01/queryPersonnelList", "person",["guaranteeNum"]),
