$(function() {
	$("#SUBMIT").unbind('click').on('click',function(event) {
		...
		var id = IPLAT.EFInput.value($("#id")); /* 宿舍人员绑定关系表id */
		var manId = IPLAT.EFInput.value($("#manId")); /* 人员id */
		var actualRent = IPLAT.EFInput.value($("#actualRent")); /* 实际房租 */
		var actualManageFee = IPLAT.EFInput.value($("#actualManageFee")); /* 实际管理费 */
		var actualInDate = IPLAT.EFInput.value($("#actualInDate")); /* 实际入住时间 */
		// 参数校验
		if (!validate(actualRent,actualManageFee,actualInDate)) {
			return;
		}
		eiInfo.set("idList", id);
		eiInfo.set("manIdList", manId);
	...
	// 参数校验
	function validate(actualRent,actualManageFee,actualInDate) {
		// var reg = /^[0-9]*$/;
		// 非负数正则
		var reg = /^(\+)?\d+(\.\d+)?$/;
