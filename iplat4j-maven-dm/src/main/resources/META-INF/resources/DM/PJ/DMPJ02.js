							...
							$("#addNode").unbind('click').click(function () {
								...
								if (validator.validate()) {
									...
									if (classifyName == null || classifyName == "") {
										NotificationUtil("宿舍满意度评价分类名称不得为空", "error");
										return;
									}
									...
									EiCommunicator.send("DMPJ02", "insert", eiInfo, {
										onSuccess: function () {
						...
						if (selectedType === "delete") {
							...
							WindowUtil({
								...
								ok: function () {
									...
									eiInfo.set("id", id);
									eiInfo.set("parentId",parentId);
									EiCommunicator.send("DMPJ02", "deleteBatch", eiInfo, {
							...
							$("#editNode").unbind('click').click(function () {
								...
								if (validator.validate()) {
									...
									if (EclassifyName == null || EclassifyName == "") {
										NotificationUtil("宿舍满意度评价项目分类名称不得为空", "error");
										return;
									}
									...
									model2Ei.set("id", id);
									EiCommunicator.send("DMPJ02", "update", model2Ei, {
										onSuccess: function () {
											// 重新加载父结点信息
											_tree.reload(model[NODE.parentIdField]);
			...
			loadComplete: function (grid) {
				$("#ADD").on("click", function (e) {
					...
					if (moduleId == null || moduleId == "") {
						NotificationUtil("请选择项目分类添加宿舍满意度评价项目,分类信息节点不能新增项目", "error");
						return;
					}
					var url = IPLATUI.CONTEXT_PATH + "/web/DMPJ0201?moduleId="+moduleId;
					var popData = $("#popData");
				...
				$("#EDIT").on("click", function (e) {
					...
					var url = IPLATUI.CONTEXT_PATH + "/web/DMPJ0202?id="+id;
				...
				$("#DELETEROW").on("click", function (e) {
					...
					IPLAT.confirm({
						message: '<b>将会删除所选中的项目行！</br><font color="red">是否确定？</font></b>',
						okFn: function (e) {
							var info=new EiInfo();
							info.set("list",arr);
							EiCommunicator.send("DMPJ02", "deleteItem", info, {
