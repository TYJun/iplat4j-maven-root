 ...
 * 满意度评价查询页面service
 *
 * @Title: ServiceDMPJ01.java
 * @ClassName: ServiceDMPJ01
 ...
 */
public class ServiceDMXSPJ01 extends ServiceBase {
    /**
     * 页面内查询接口.
    ...
    /**
     * 获取登录人信息，判断角色值，查询宿舍满意度信息
     * 1、获取当前登录人信息，如果登录人不存在，提示错误信息.
     * 2.获取当前登录人的用户角色，根据用户角色对view赋予它的角色值，以便后续查工单列表的时候做数据显示角色区分.
     * 3.调用本地服务DMPJ01.queryPJInfoList()方法进行列表数据查询.
     * @param inInfo
     * @return
     */
    public EiInfo query(EiInfo inInfo) {
        ...
        // 调用本地服务DMPJ01.queryPJInfoList()方法进行列表数据查询.
        inInfo.set(EiConstant.serviceName, "DMPJ01");
        inInfo.set(EiConstant.methodName, "queryPJInfoList");
        EiInfo outInfo = XLocalManager.call(inInfo);
        return outInfo;
    }

    /**
     * 根据前端输入的工号、姓名、员工类型、科室部门、评价状态、房间类型、宿舍楼
     * 宿舍层、房间号、人员大类查询满意度信息
     ...
     * @return
     */
    public EiInfo queryPJInfoList(EiInfo inInfo) {
         ...
         * 1、将要查询的参数组成数组并调用工具类转换参数
         */
        String[] param = {"manNo", "manName","deptName", "employmentNature","typeCode","building", "floor","roomNo","evalStatus"};
        ...
        //查询
        List<Map<String, Object>> list = dao.query("DMPJ01.queryPJList",map,
                Integer.parseInt(map.get("offset").toString()),Integer.parseInt(map.get("limit").toString()));
        int count = super.count("DMPJ01.countPJList",map);
        // 判断是否存在，存在则构建返回对象
