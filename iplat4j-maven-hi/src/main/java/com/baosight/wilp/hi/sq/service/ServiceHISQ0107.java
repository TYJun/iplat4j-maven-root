 ...
 * <desc>    // 描述修改内容
 */
public class ServiceHISQ0107 extends ServiceBase {
	/**
	 * @param inInfo id 主键
	 *               type 操作类型
	 * @return EiInfo
	...
	public EiInfo evaluationOverrule (EiInfo inInfo) {
		...
		if("null".equals(id)){
			...
			if("01".equals(status)){
				...
				dao.insert("HISQ0101.insert", approvalRecords);
				//3、保存审批状态
				dao.update("HISQ01.evaluationOverrule4", map);
			}
		}
		//4、返回参数
		return inInfo;
