 ...
 * <desc>    // 描述修改内容
 */
public class ServiceHISQ0104 extends ServiceBase {
	/**
	 * @param inInfo id 主键
	 *               type 操作类型
	 * @return EiInfo
	...
	public EiInfo evaluationPass (EiInfo inInfo) {
		...
		pMap1.put("fileStatus", "03");
		List<HiFile> fileDate = dao.query("HIBZ01.queryId", pMap1);
		EiBlock block1 = new EiBlock("resultD");
		block1.setRows(fileDate);
		inInfo.setBlock(block1);
		...
		if("null".equals(id)){
           ...
           if("01".equals(status)){
			   ...
			   //3、保存审批状态、制作状态
			   dao.update("HISQ01.evaluationPass1", map);
		   }
		}
		//保存附件
		saveFile.saveFile(fileObj, id, deleteFile, status);
//		//改变文件的状态
//		Map<String, String> pMap11 = new HashMap<>(4);
//		pMap11.put("id",id);
//		pMap11.put("fileStatus", "05");
	...
	public EiInfo evaluationOverrule (EiInfo inInfo) {
		...
		if("null".equals(id)){
			...
			if("01".equals(status)){
				...
				dao.insert("HISQ0101.insert", approvalRecords);
				//3、保存审批状态
				dao.update("HISQ01.evaluationOverrule1", map);
			}
		}
		//4、返回参数
		return inInfo;
