$(function() {
    ...
    var id = __ei.id;
    //标识名称
    var iconName =__ei.iconName;
    IPLAT.EFPopupInput.value($("#iconName"),iconName);
    // 标识分类
    ...
    IPLAT.EFPopupInput.value($("#inqu_status-0-spotName"),fixedPlaceNum);
    IPLAT.EFPopupInput.text($("#inqu_status-0-spotName"),fixedPlaceName);
    ...
    // 使用科室
    var deptNum = __ei.deptNum;
    var deptName = __ei.deptName;
    IPLAT.EFPopupInput.value($("#inqu_status-0-deptName"),deptNum);
    IPLAT.EFPopupInput.text($("#inqu_status-0-deptName"),deptName);
...
//文件下载
function selectStandentFile() {
    var checkRows = resultDGrid.getCheckedRows();
    if (checkRows.length > 0) {
        for (var index in checkRows) {
            var id = checkRows[index].id;
            var url = IPLATUI.CONTEXT_PATH + "/web/HIBZ0103?initLoad&id="+id +"&type="+ "see";
            // window.location.href = href;
            let popData = $("#popWorldData");
            ...
            popWorldDataWindow.open().center();
        }
    } else {
...
function validatePR(eiInfo) {
    if (isEmpty(eiInfo.get("iconName"))) {
