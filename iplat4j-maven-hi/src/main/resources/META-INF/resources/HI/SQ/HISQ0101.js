...
$(function () {
    ...
    $("#SAVEPR").on("click", function () {
        ...
        var spotNum = $("#spotNum").val();	/* 维修地点编码*/
        var spotName = $("#spotName").val();	/* 维修地点*/
        ...
        eiInfo.set("spotNum", spotNum)
        eiInfo.set("spotName", spotName)
        if (!validatePR(eiInfo)) {
            return;
        }
        //提交
        EiCommunicator.send("HISQ0101", "saveApplicationInformation", eiInfo, {
            onSuccess: function (ei) {
                closeCurrentWindow();
...
function validatePR(eiInfo) {
    ...
    //     IPLAT.NotificationUtil("标识中文名称不能为空","error");
    //     return false;
    // }
    //
    // if (isEmpty(eiInfo.get("inqu_status-0-iconEnContent"))) {
    //     IPLAT.NotificationUtil("标识英文名称不能为空","error");
    //     return false;
    // }

    if (isEmpty(eiInfo.get("inqu_status-0-classifyId"))) {
        IPLAT.NotificationUtil("标识分类不能为空","error");
        return false;
    }

    if (isEmpty(eiInfo.get("spotName"))) {
        IPLAT.NotificationUtil("安装地点不能为空","error");
        return false;
    }

    if (isEmpty(eiInfo.get("inqu_status-0-iconAmount"))) {
        IPLAT.NotificationUtil("数量不能为空","error");
        return false;
    }


    if (isEmpty(eiInfo.get("inqu_status-0-spotDesc"))) {
        IPLAT.NotificationUtil("安装地点说明不能为空","error");
        return false;
    }

    if (isEmpty(eiInfo.get("inqu_status-0-applyReason"))) {
        IPLAT.NotificationUtil("申请理由不能为空","error");
        return false;
    }

    // if (isEmpty(eiInfo.get("inqu_status-0-remark"))) {
    //     IPLAT.NotificationUtil("备注不能为空","error");
    //     return false;
    // }
    if (isEmpty(eiInfo.get("inqu_status-0-applyDeptName"))) {
        IPLAT.NotificationUtil("申请科室不能为空","error");
        return false;
    }
