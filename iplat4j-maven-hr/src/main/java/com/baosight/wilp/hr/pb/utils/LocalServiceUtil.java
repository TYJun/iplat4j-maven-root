...
//	}
	 ...
	 * @return:
	 */
	public static <T>T getObject(Map<String, Object> map, Class<T> c) throws Exception {
	 ...
	 * @return:
	 */
	public static <T>T copyObject(Map<String, Object> map, T c) throws Exception {
		...
		for (Map.Entry<String, Object> entry : entries) {//获取集合里面的元素
			...
			//2.将map中的值存入T这个类的对象属性中
			try {
				Field f = clazz.getDeclaredField(key);//获取类的所有字段
				f.setAccessible(true);//简单的理解：设置访问权限
				f.set(c,entry.getValue());//给T对象赋值
	...
	public static void main(String[] args) throws Exception {
		...
		map.put("respCode","111");
		map.put("respMsg","122");
		r.setTotal(100);
		r.setSuccess(false);
		ResultData a = copyObject(map, r);
		System.out.println(a);

		Integer s = null;
		int cardReChargePrice = Integer.parseInt(s.toString());
