...
/**
 * 该页面为设备保洁管理
 * 主要包含对设备保洁信息的页面查询、删除、设备查询、保洁单位查询、负责人查询
 * <p>1.初始化查询 initLoad
 * <p>2.保洁数据查询 query
 * <p>3.保洁数据删除 delete
 ...
 * @ClassName: ServiceBFDJ01
 * @Package：com.baosight.wilp.bf.dj.service
 * @author: liangyongfei
 ...
 * <desc>    // 描述修改内容
 */
public class ServiceDFBJ01 extends ServiceBase {
    /**
     * @Title: initLoad
     * @Description: 初始化查询
     * 设备保洁管理界面
     * 通过保洁单号、设备名称、安装地点、保洁日期起、保洁日期止条件查询
     * 回显保洁单号、设备名称、安装地点、保洁单位、负责人、保洁日期、作业说明
     * @param inInfo
     * cleanNo 保洁单号
     * machineName 设备名称
     * fixedPlace 安装地点
	...
	public EiInfo query(EiInfo inInfo) {
	    // 创建数组并赋值
		String[] parameter = {"cleanNo","machineName","fixedPlace","cleanDateS","cleanDateE"};
		// 数组转list
		List<String> plist = Arrays.asList(parameter);
		// 调用接口创建map
		Map<String, Object> map = CommonUtils.buildParamter(inInfo, "result", plist);
		// 调用查询方法
		List<Map<String, ?>> result = dao.query("DFBJ01.query", map,Integer.parseInt(map.get("offset").toString()),Integer.parseInt(map.get("limit").toString()));
		// 获取查询信息总数
		int count = dao.count("DFBJ01.query", map);
		// 如果获取查询信息不为空
	...
	public EiInfo delete(EiInfo inInfo) {
	    // 调用删除方法
		return super.delete(inInfo, "DFBJ01.delete");
	}
