$(function() {
	...
	$("#COMFIRM").unbind('click').on('click', function(e){
		...
		if(validate(rows)){
			...
			EiCommunicator.send("CICG0101", "insert", eiInfo, {
...
/*function popDataCY(deptNum,deptName) {
	var url = IPLATUI.CONTEXT_PATH + "/web/CICG0103?initLoad&deptNum=" + deptNum +"&deptName=" + deptName;
	var popData = $("#popDataCY");
	popData.data("kendoWindow").setOptions({
		open : function() {
			popData.data("kendoWindow").refresh({
				url : url,
