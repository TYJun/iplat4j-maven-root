$(function() {
	...
	$("#COMFIRM").unbind('click').on('click', function(e){
		...
		var canteenNum = IPLAT.EFPopupInput.value($("#canteen"));
		var canteenName = IPLAT.EFPopupInput.text($("#canteen"));
		var rows = resultGrid.getDataItems();
		var id = IPLAT.EFInput.value($("#id"));
		var applyBillNo = IPLAT.EFInput.value($("#applyBillNo"));
		//参数检验
		if(validate(canteenName,rows)){
			...
			EiCommunicator.send("CISQ0102", "update", eiInfo, {
...
//参数校验
function validate(canteenName,rows) {
	if(canteenName == null || canteenName=="" || canteenName==undefined){
