		...
		"result": {
			loadComplete: function (grid) {
				...
				$("#ADD").on("click", function (e) {
					var url = IPLATUI.CONTEXT_PATH + "/web/CISQ0101";
					var popData = $("#popData");
				...
				$("#REEDIT").on("click", function (e) {
					...
					for (var i = 0; i < rows.length; i++) {
						if(rows[i].applySign == "已提交"){
							NotificationUtil("不能编辑已提交的单据", "error");
							return;
						}
					}
					if(rows.length > 0){
						id = rows[0].id;
						canteenName=rows[0].applyCanteenName;
					...
					var url = IPLATUI.CONTEXT_PATH + "/web/CISQ0102?id="+id+"&canteenName=" +canteenName+"&applyBillNo=" + applyBillNo +"&emo=" + emo;
				...
				$("#REDELETE").on("click",function(e) {
					...
					for (var i = 0; i < checkRows.length; i++) {
						if(checkRows[i].applySign == "已提交" ){
							NotificationUtil("已提交的单据不能删除", "error");
							return;
						}
					...
					IPLAT.confirm({
						...
						okFn: function (e) {
							EiCommunicator.send("CISQ01", "delete", info, {
				...
				$("#SUBMIT").on("click", function (e) {
					...
					EiCommunicator.send("CISQ01", "updateSubmit", inInfo, {
						onSuccess : function(ei) {
...
function popData(outBillNo, outTypeName, wareHouseName, userDeptName) {
	var url = IPLATUI.CONTEXT_PATH + "/web/CISQ0103?initLoad&outBillNo=" + outBillNo
