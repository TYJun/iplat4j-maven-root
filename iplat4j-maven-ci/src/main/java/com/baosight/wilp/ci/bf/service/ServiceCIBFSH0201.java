 ...
 */
public class ServiceCIBFSH0201 extends ServiceBase {
    ...
    public EiInfo initLoad(EiInfo inInfo) {
        ...
        pMap.put("outBillNo", inInfo.getString("applyBillNo"));
    ...
    public EiInfo updateApprove(EiInfo inInfo) {
         ...
         *1.对id对应的报废出库单进行出库
         */
        List<CiOut> list = dao.query("CICK01.query", map);
        if(list==null || list.size()<=0){
            //返回
            inInfo.setStatus(-1);
            inInfo.setMsg("报废出库单不存在！");
            return inInfo;
        }

        Map map2=new HashMap();
        map2.put("outBillNo",list.get(0).getOutBillNo());
        List<CiOutDetail> outDetailList=dao.query("CICK0101.query",map2);
        if(outDetailList==null || outDetailList.size()<=0){
            inInfo.setStatus(-1);
            inInfo.setMsg("报废出库单明细不存在或没有数据！");
            return inInfo;
        }

        //对所有的出库单进行出库
        EiInfo outS=new EiInfo();
        outS.set("out",list.get(0));
        outS.set("outDetailList",outDetailList);
        outS.set(EiConstant.serviceName, "CICK0101");
        outS.set(EiConstant.methodName, "outStockByCommon");
        EiInfo r=XLocalManager.call(outS);
        if(r==null || r.getStatus()!=0){
            inInfo.setStatus(-1);
            inInfo.setMsg("操作报废出库处理失败："+r.getMsg());
            return inInfo;
        }

        //更新工单状态
        int count = dao.update("CICK01.updateApplySignS", map);
        if (count > 0) {
            inInfo.setStatus(1);
            inInfo.setMsg("审核成功");
            return inInfo;
    ...
    public EiInfo updateReject(EiInfo inInfo) {
        ...
        //更新工单状态
        int count = dao.update("CICK01.updateReject", map);
        if (count > 0) {
            inInfo.setStatus(1);
            inInfo.setMsg("驳回成功");
            return inInfo;
