$(function() {
	...
	if(op === "see"){
		IPLATUI.EFGrid = {
			"result": { pageable : false, exportGrid : true, },
		}
	} else {
		IPLATUI.EFGrid = {
			"result": {
	...
	$("#COMFIRM").unbind('click').on('click', function(e){
		...
		if(flag){
			...
			EiCommunicator.send("SIDB0101", "saveStockTrans",eiInfo, {
...
function popData(outWareHouseNo) {
	var url = IPLATUI.CONTEXT_PATH + "/web/SIDB0102?initLoad&inqu_status-0-outWareHouseNo=" + outWareHouseNo;
