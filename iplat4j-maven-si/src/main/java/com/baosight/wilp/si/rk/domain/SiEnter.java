...
* 
*/
public class SiEnter extends DaoEPBase {
    ...
    /**总入库数量**/
    private Double totalNum = new Double(0d);
    /**总入库金额**/
    private Double totalAmount = new Double(0d);
    ...
    /** 打印标记(0=未打印,1=已打印)*/
    private Integer printFlag = new Integer(0);
    ...
    public void initMetaData() {
        ...
        eiColumn = new EiColumn("checkSign");
        eiColumn.setDescName("验收签字图片");
        eiMetadata.addMeta(eiColumn);

        eiColumn = new EiColumn("approvalSign");
        eiColumn.setDescName("审核签字图片");
        eiMetadata.addMeta(eiColumn);
	...
	public void fromMap(Map map) {
        ...
        setEnterBillNo(SiUtils.isEmpty(map.get("enterBillNo"), enterBillNo));
        setEnterType(SiUtils.isEmpty(map.get("enterType"), enterType));
        setEnterTypeName(SiUtils.isEmpty(map.get("enterTypeName"), enterTypeName));
        setOriginBillNo(SiUtils.isEmpty(map.get("originBillNo"), originBillNo));
        setOriginBillType(SiUtils.isEmpty(map.get("originBillType"), originBillType));
        setOriginBillTypeName(SiUtils.isEmpty(map.get("originBillTypeName"), originBillTypeName));
        setContNo(SiUtils.isEmpty(map.get("contNo"), contNo));
        setPurcDemandDetailNo(SiUtils.isEmpty(map.get("purcDemandDetailNo"), purcDemandDetailNo));
        ...
        setBillMakeTime(SiUtils.isEmpty(map.get("billMakeTime"), billMakeTime));
        setBillMaker(SiUtils.isEmpty(map.get("billMaker"), billMaker));
        setBillCheckTime(SiUtils.isEmpty(map.get("billCheckTime"), billCheckTime));
        setBillChecker(SiUtils.isEmpty(map.get("billChecker"), billChecker));
        setInvoiceNum(SiUtils.isEmpty(map.get("invoiceNum"), invoiceNum));
        setIsCheck(NumberUtils.toInteger(StringUtils.toString(map.get("isCheck")), isCheck));
        setBillCheckerName(SiUtils.isEmpty(map.get("billCheckerName"), billCheckerName));
        setBillMakerName(SiUtils.isEmpty(map.get("billMakerName"), billMakerName));
        setBatchNo(SiUtils.isEmpty(map.get("batchNo"), batchNo));
        setRemark(SiUtils.isEmpty(map.get("remark"), remark));
        setSupplierNum(SiUtils.isEmpty(map.get("supplierNum"), supplierNum));
        setSupplierName(SiUtils.isEmpty(map.get("supplierName"), supplierName));
        setPrintFlag(NumberUtils.toInteger(StringUtils.toString(map.get("printFlag")), printFlag));
        setCheckSign(SiUtils.isEmpty(map.get("checkSign"), checkSign));
        setApprovalSign(SiUtils.isEmpty(map.get("approvalSign"), approvalSign));
        setUserDeptNum(SiUtils.isEmpty(map.get("userDeptNum"), userDeptNum));
        setUserDeptName(SiUtils.isEmpty(map.get("userDeptName"), userDeptName));
    }
	...
	public Map toMap() {
        ...
        map.put("checkSign",StringUtils.toString(checkSign, eiMetadata.getMeta("checkSign")));
        map.put("approvalSign",StringUtils.toString(approvalSign, eiMetadata.getMeta("approvalSign")));
