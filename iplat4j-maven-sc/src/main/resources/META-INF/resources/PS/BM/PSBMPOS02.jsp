       ...
       <div title="待处理">
        <EF:EFRegion id="result" title="记录集">
            <EF:EFGrid blockId="result" autoDraw="no" rowNo="true" readonly="true" checkMode="multiple, cell">
                ...
                <EF:EFColumn ename="cz" displayType="url" cname="操作" width="100" align="center" readonly="true"/>
            </EF:EFGrid>
        </EF:EFRegion>
       </div>
       <div title="已处理">
         <EF:EFRegion id="result2" title="记录集">
            <EF:EFGrid blockId="result2" serviceName="PSBMPOS02" queryMethod="query2" autoDraw="no" rowNo="true" readonly="true" >
                <!-- 隐藏列 -->
                <EF:EFColumn ename="id" cname="id" width="100" hidden="true" />
                <EF:EFColumn ename="isDelete" cname="isdelete" width="100" hidden="true" />
                <!-- 展示列 -->
        ...
        dataBinding: function (e) {
            ...
            for (var i = 0, length = e.items.length; i < length; i++) {
                if(isAvailable(e.items[i].rejectCode)){
                	e.items[i].address = e.items[i].deptName + e.items[i].bedNo;
...
function formatOrderCz(row){
    ...
    if(code=="1"){
    	//POS订单作废操作与病员订单作废相同
        return "<a style='color:blue;' href=\"#\" onclick=\"operationPatientOrder('"+id+"','PSBMDDZF01?confirmCancelOrder','确认作废');\">确认作废</a>";;
    }
}
//订单操作事件
