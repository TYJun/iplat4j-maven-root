...
<%--             <EF:EFSelect ename="inqu_status-0-spotName" cname="所属楼栋" 
            ...
            <EF:EFInput ename="inqu_status-0-patientName" colWidth="3" ratio="3:8" cname="职工姓名" />
        </div>
    </EF:EFRegion>
       ...
       <div title="待处理">
        <EF:EFRegion id="result" title="记录集">
            <EF:EFGrid blockId="result" autoDraw="no" rowNo="true" readonly="true" checkMode="multiple, cell">
                ...
                <EF:EFColumn ename="payType" cname="付款方式" width="100" align="center" readonly="true" hidden="true" />
                <!-- 展示列 -->
...
<%--                <EF:EFColumn ename="reqSendTime" cname="取餐时间" width="80" align="center" readonly="true"/>--%>
                ...
                <EF:EFColumn ename="recReviseTime" cname="作废时间" width="100" align="center" readonly="true"/>
                <EF:EFColumn ename="rejectReason" cname="作废原因" width="100" align="center" readonly="true"/>
       ...
       <div title="已处理">
         <EF:EFRegion id="result2" title="记录集">
            <EF:EFGrid blockId="result2" serviceName="SSDCZQZF01" queryMethod="query2" autoDraw="no" rowNo="true" readonly="true" >
                <!-- 隐藏列 -->
                <EF:EFColumn ename="id" cname="id" width="100" hidden="true" />
                <EF:EFColumn ename="isDelete" cname="isdelete" width="100" hidden="true" />
                <EF:EFColumn ename="payType" cname="付款方式" width="100" align="center" readonly="true" hidden="true" />
...
IPLATUI.EFGrid = {
    "result": {
        dataBinding: function (e) {
            ...
            for (var i = 0, length = e.items.length; i < length; i++) {
                if(isAvailable(e.items[i].rejectCode)){
                    e.items[i].cz = formatOrderCz(e.items[i]);
                    if(e.items[i].rejectCode == "1"){
                        e.items[i].statusName = '作废待确认';
                    };
    ...
    "result2": {
        dataBinding: function (e) {
            //grid数据绑定时对属性进行处理
            for (var i = 0, length = e.items.length; i < length; i++) {
                if(isAvailable(e.items[i].rejectCode) && e.items[i].rejectCode == "2"){
                    e.items[i].statusName = '已确认';
                };
                if(e.items[i].payType == "0000"){
                    e.items[i].payTypeName = '现金';
                };
...
function formatOrderCz(row){
    var htm = "";
    var id = row.id;
    var code = row.rejectCode;
    if(code=="1"){
        return "<a style='color:red;' href=\"#\" onclick=\"operationPatientOrder('"+id+"','SSDCZQZF01?confirmCancelOrder','确认作废');\">确认作废</a>";
    }
}
//订单操作事件
...
function setDate(){
    $("#inqu_status-0-beginDate").data("kendoDatePicker").value("2021-04-01");
