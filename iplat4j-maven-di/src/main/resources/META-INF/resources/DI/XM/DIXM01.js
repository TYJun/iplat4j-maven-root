						...
						if (selectedType === "add") {
							...
							$("#addNode").unbind('click').click(function () {
								...
								if (validator.validate()) {
									...
									EiCommunicator.send("DIXM01", "insert", eiInfo, {
										onSuccess: function () {
						...
						if (selectedType === "delete") {
							...
							WindowUtil({
								...
								ok: function () {
									...
									eiInfo.set("id", id);
									eiInfo.set("parentId",parentId);
									EiCommunicator.send("DIXM01", "deleteBatch", eiInfo, {
						...
						if (selectedType === "edit") {
							...
							$("#editNode").unbind('click').click(function () {
								...
								if (validator.validate()) {
									...
									model2Ei.set("id", id);
									EiCommunicator.send("DIXM01", "update", model2Ei, {
										onSuccess: function () {
											// 重新加载父结点信息
											_tree.reload(model[NODE.parentIdField]);
		...
		"result": {
			loadComplete: function (grid) {
				$("#ADD").on("click", function (e) {
					...
					var url = IPLATUI.CONTEXT_PATH + "/web/DIXM0101?moduleId="+moduleId;
					var popData = $("#popData");
				...
				$("#EDIT").on("click", function (e) {
					...
					var url = IPLATUI.CONTEXT_PATH + "/web/DIXM0102?id="+id;
				...
				$("#DELETEROW").on("click", function (e) {
					...
					var info=new EiInfo();
					info.set("list",arr);
					EiCommunicator.send("DIXM01", "deleteItem", info, {
