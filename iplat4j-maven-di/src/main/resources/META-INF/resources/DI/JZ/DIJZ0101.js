			...
			"device": {
				...
				loadComplete: function (grid) {
					...
					$("#ADDDEVICE").on("click", function (e) {
						var url = IPLATUI.CONTEXT_PATH + "/web/DIJZ010101";
						var popData = $("#popDataDevice");
						if(operType=="update"){
							NotificationUtil("设备信息不允许编辑！", "error");
						}else{
					...
					$("#DELETEDEVICE").click(function() {
						if(operType=="update"){
							NotificationUtil("设备信息不允许删除！", "error");
						}else{
					...
					$("#ADDPACDEVICE").on("click", function (e) {
						if(operType=="update"){
							...
							var url = IPLATUI.CONTEXT_PATH + "/web/DIJZ010105";
			...
			"cycle": {
				...
				loadComplete: function (grid) {
					$("#ADDCYCLE").on("click", function (e) {
						var url = IPLATUI.CONTEXT_PATH + "/web/DIJZ010102";
...
//						var cycle = checkRows[0].id;
			...
			"item": {
				...
				loadComplete: function (grid) {
					$("#ADDITEM").on("click", function (e) {
						...
						var url = IPLATUI.CONTEXT_PATH + "/web/DIJZ010103?schemeID="+schemeID;
					...
					$("#ADDCARD").click(
						function() {
							var url = IPLATUI.CONTEXT_PATH + "/web/DIJZ010106?"
									+ "initLoad";
			...
			"exman": {
				...
				loadComplete: function (grid) {
					$("#ADDEXMAN").on("click", function (e) {
						...
						var url = IPLATUI.CONTEXT_PATH + "/web/DIJZ010104?deptName="+deptName;
	...
	$("#SUBMIT").click(function() {
		...
		for(var i=0;i<cycleList.length;i++){
			var startTime = kendo.toString(cycleList[i].startTime, "yyyy-MM-dd HH:mm:ss");
			//校验周期
			if(cycleList[i].cycle==""){
				NotificationUtil("第"+(i+1)+"行的周期不能为空", "error");
				return;
			}
			var regu = /^[1-9]\d*$/;
			if(!regu.test(cycleList[i].cycle)){
				NotificationUtil("第"+(i+1)+"行的周期为正整数,请重输", "error");
				return;
			}
			//校验时间
			...
			if(startTime<data){
				NotificationUtil("第"+(i+1)+"行的开始时间不能小于当前时间", "error");
				return;
			}
			cycleList[i].startTime=startTime;
		}
		...
		EiCommunicator.send("DIJZ0101", "insertScheme", info, {
