...
public class ServiceDIJZ01 extends ServiceBase{
	...
	public EiInfo initLoad(EiInfo inInfo) {
        ...
        if(userInfo !=null){
            inInfo.set("deptName", userInfo.get("deptName"));
    ...
    public EiInfo query(EiInfo inInfo) {
        ...
        String[] param = {"schemeID", "schemeCode", "schemeName", "schemeDept", "machineCode", "machineName", "status","fixedPlace","deptName"};
        //将参数封装并获取分页，执行查询
        Map<String, Object> map = DeviceEiUtil.buildParam(inInfo, Arrays.asList(param), "result");
        List<Map<String, String>> list = dao.query("DIJZ01.queryScheme", map);
        ...
        if(null != list || list.size()>0) {
            for (int i = 0; i < list.size(); i++) {
                ...
                if(null != packetID) {
                    List<Map<String,String>> query = dao.query("DIJZ01.queryPacket", param2);
        ...
        int count = dao.count("DIJZ01.countScheme", map);
    ...
    public EiInfo deleteScheme(EiInfo inInfo) {
    	...
    	List<Map<String,String>> isTask=dao.query("DIJZ01.isTask", ids);
    	if(!isTask.isEmpty()) {
    	    inInfo.setMsg("该基准已生成任务,不能删除");
            return inInfo;
    	}
    	//删除基准
    	dao.delete("DIJZ01.deleteScheme", ids);
    	//循环删除执行周期、基准项目、执行人
    	for (String id : ids) {
			dao.delete("DIJZ01.deleteCycle", id);//删除执行周期
			dao.delete("DIJZ01.deleteItem", id);//删除基准项目
			dao.delete("DIJZ01.deleteExman", id);//删除执行人
		}
    	inInfo.setMsg("删除成功");
    	return inInfo;
    }
    ...
    public EiInfo startScheme(EiInfo inInfo) {
    	...
    	list.forEach(schemeID -> {
    		...
    		dao.update("DIJZ01.updateScheme", param);
    	});
