...
import com.baosight.wilp.di.common.util.DevicePicUtil;
 ...
 * <desc>    // 描述修改内容
 */
public class ServiceDIJZApp extends ServiceBase {

    @Autowired
    patrolLogicService patrolLogicService;
    /**
     * 图片保存路径
     */
    public static final String PIC_PATH = "upload/device";

    private static final Logger logger = Logger.getLogger(ServiceDIJZApp.class);
...
//			pMap.put("respMsg", "userID参数为空");
            ...
            try {
				/*int count= dao.count("DIJZ01.getDevExceptionTaskAppCount", map);
				//判断总数是否小于startRow
				if(count<=startRow) {
					startRow=count-rows;
					map.put("startRow", startRow);
				}*/
                List list = dao.query("DIJZ01.getDevExceptionTaskApp", map);
...
//		     }
        ...
        try {
            ...
            List<Map<String, Object>> query = dao.query("DIJZ01.getDeviceWorkloadApp", map);
            /*
...
//        return queryOffTask(inInfo);
    ...
    public EiInfo getDeviceDetail(EiInfo inInfo) {
        ...
        List<Map<String, Object>> list = dao.query("DIJZ01.getMachineInfo", inInfo.getString("id"));
     ...
     */
    @SuppressWarnings("unchecked")
    public EiInfo queryOffTask(EiInfo inInfo) {
        ...
        if("N".equals(inInfo.getString("isPage"))){
            ...
            count = dao.count("DIJZ01.queryTaskCount", map);
        }
        //改造获取完成人员名称 -- 2021-08-04
//        List<Map<String, Object>> list = dao.query("DIJZ01.queryTask", map);
        List<Map<String, Object>> list = dao.query("DIJZ01.queryTaskBySchemeID", map);
//        List<Map<String, Object>> list = dao.query("DIJZ01.queryTaskitemAll", map);

        List<Map<String, Object>> rList = new ArrayList<>();
        list.forEach(stringObjectMap -> {
            List<Map<String, Object>> plist = dao.query("DIJZ01.queryTaskitemByTaskID", stringObjectMap);
            rList.addAll(plist);
        });

        System.out.println("rList大小--------"+rList.size());

        //获取NFC或扫描类型用于APP判断（判断是NFC刷卡还是扫二维码）
        String scanType = CommonUtils.getDataCodeItemName("wilp.di.config", "01");
        inInfo.set("scanType", scanType);
        //获取码类型型用于APP判断（判断扫设备二维码还是地点二维码）
        String codeType = CommonUtils.getDataCodeItemName("wilp.di.config", "02");
        inInfo.set("codeType", codeType);
...
//        List<Map<String, Object>> rList =  dealResult(list);
    ...
    public EiInfo queryTask(EiInfo inInfo) {
        ...
        logger.info("巡检离线任务前台传参=================" + map.toString());
        //改造获取完成人员名称 -- 2021-08-04
        List<Map<String, Object>> list = dao.query("DIJZ01.queryTask", map);
        //获取NFC或扫描类型用于APP判断（判断是NFC刷卡还是扫二维码）
        String scanType = CommonUtils.getDataCodeItemName("wilp.di.config", "01");
        ...
        //循环将任务项目数据处理出来
        List<Map<String, Object>> rList =  dealResult(list);
        logger.info("巡检离线任务返回数=================" + rList.size());
//        List<Map<String, Object>> rList =  dealResultOff(list);
        count = "N".equals(inInfo.getString("isPage")) ? list == null ? 0 : list.size() : count;
        return DeviceEiUtil.buildResult(inInfo, rList, count, "result");
    }
    ...
    private List<Map<String, Object>> dealResult(List<Map<String, Object>> list) {
        ...
        list.forEach(map -> {
            ...
            pMap.put("sortNo", map.get("sortNo"));
            plist.add(pMap);
     ...
     */
    @SuppressWarnings("unchecked")
    private List<Map<String, Object>> dealResultOff(List<Map<String, Object>> list) {
        List<Map<String, Object>> result = new ArrayList<Map<String,Object>>();
        Map<String, Object> temp = new HashMap<String, Object>();
    ...
    public EiInfo commitXJ(EiInfo inInfo) throws ParseException {
        ...
        for (int i = 0; i < jsonArr.size(); i++) {
            ...
            updateDeviceTask(StringUtils.isBlank(finishTimes) ? new Date() : new SimpleDateFormat("yyyy-MM-dd HH:mm").parse(finishTimes),
                    (String)workInfo.get("workNo"),(String)workInfo.get("name"), "Y".equals(closeFlag) ? "3" : "2", exceptionFlag, taskID);
        }
    ...
    public EiInfo handDevExceptionTask(EiInfo inInfo) {
        ...
        dao.update("DIJZ01.handDevExceptionTask",map);
    ...
    private boolean selecTaskStatus(String taskID){
        Map<String, Integer> map = (Map<String, Integer>) dao.query("DIJZ01.getTaskStatus" ,taskID).get(0);
     ...
     * @return: void
     */
    public void updateDeviceTask(Date finishTime, String workNo, String finishMan, String status, String exceptionFlag, String taskID) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Map<String, Object> param = new HashMap<>();
        //如果任务已完成，录入完成人名称
        if(status.equals("2")){
            param.put("finishTime", format.format(finishTime));
            param.put("finishMan", workNo);
        ...
        dao.update("DIJZ01.updateDeviceTask", param);
    }
    ...
    public void updateTaskItem(String itemID, String errorContent, String writeValue,
        ...
        dao.update("DIJZ01.updateTaskItem", param);
    }
    ...
    public Boolean uploadPic(Map<String,String> param,String pic) {
        ...
        dao.insert("DIJZ01.insertPic", map);
        return true;
    }
    ...
    public String getUserRole(String name) {
        ...
        List<String> list = dao.query("DIJZ01.getUserRole",pMap);
        String roles = "";
        if(list.size() > 0 ) {
            roles = list.get(0);
        }
    ...
    public EiInfo queryTaskExc(EiInfo inInfo) {
        ...
        if("N".equals(inInfo.getString("isPage"))){
            ...
            count = dao.count("DIJZ01.queryTaskCountExc", map);
        }

        //改造获取完成人员名称 -- 2021-08-04
        List<Map<String, Object>> list = dao.query("DIJZ01.queryTaskExc", map);
    ...
    /****
     * 通过任务名称查询倒计时时间
     * 传参：configureName ：任务名称
     * 返回：
     * isTime：N 直接保存
     * isTime：Y，获取Time(秒)进行倒计时
     * */
    public EiInfo queryconfigureTime(EiInfo inInfo) {
        String param  = inInfo.getString("configureName");
        List<Map<String, Object>> list = dao.query("DIJZ03.queryconfigureTime", new HashMap(){{
            put("configureName",param);
        }});
        if (list.size()>0){
         inInfo.set("isTime","Y");
         inInfo.set("Time",list.get(0).get("configureTime"));
        }else {
            inInfo.set("isTime","N");
        }
        return inInfo;
 ...
 *      workAround：解决措施
 *      finishTime：实际完成时间
 *      processType：解决方式--现场解决为0,上报解决：1
 *      dealMan:处理人
 *      jobDescription:作业说明
 *      abnormalFailure：异常描述描述
 * 上报解决时：
 * 传参：itemId：项目ID
 *      processType：解决方式--现场解决为0,上报解决：1
 *      jobDescription:作业说明
 ...
 *
 */
    public EiInfo exceptionHand(EiInfo inInfo){
        String processType = inInfo.getString("processType");//解决方式
        String itemId = inInfo.getString("itemId");//项目ID
        String workAround  = inInfo.getString("workAround");//解决措施
        String finishTime = inInfo.getString("finishTime");//实际完成时间
        String dealMan = inInfo.getString("dealMan");//处理人
        String jobDescription = inInfo.getString("jobDescription");//作业说明
        String abnormalFailure = inInfo.getString("abnormalFailure");//异常故障描述
        if(DiConstant.PROCESS_TYPE_SCENE.equals(processType)){
            /******现场处理*******/
            if(StringUtils.isEmpty(itemId)||StringUtils.isEmpty(workAround)||StringUtils.isEmpty(finishTime)||StringUtils.isEmpty(processType)){
                inInfo.setStatus(-1);
                inInfo.setMsg("缺少参数");
                return inInfo;
            }
            patrolLogicService.sceneDeal(new HashMap(){{
                put("itemId",itemId);
                put("workAround",workAround);
                put("finishTime",finishTime);
                put("processType",processType);
                put("dealMan",dealMan);
                put("solutionType","现场处理");
                put("jobDescription",jobDescription);
                put("abnormalFailure",abnormalFailure);
            }});
            inInfo.setMsg("现场处理完成");
        }else if(DiConstant.PROCESS_TYPE_REPORT.equals(processType)){
            /*******上报处理***********/
            if(StringUtils.isEmpty(itemId)||StringUtils.isEmpty(processType)){
                inInfo.setMsg("缺少参数");
                inInfo.setStatus(-1);
                return inInfo;
            }
            patrolLogicService.reportDeal(new HashMap(){{
                put("itemId",itemId);
                put("processType",processType);
                put("jobDescription",jobDescription);
                put("abnormalFailure",abnormalFailure);
            }});
            inInfo.setMsg("上报处理完成");
        }
        return inInfo;
    }
}
