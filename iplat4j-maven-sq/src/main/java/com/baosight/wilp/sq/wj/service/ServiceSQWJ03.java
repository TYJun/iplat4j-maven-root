 ...
 * <desc>    // 描述修改内容
 */
public class ServiceSQWJ03 extends ServiceBase {
	/**
	 * 页面初始化方法
	 */
	...
	public EiInfo insertSQManage(EiInfo inInfo) {
		String manageId = inInfo.getString("manageId");
		manageId = manageId.substring(0,manageId.length() - 1);
		String[] manageIds = manageId.split(",");
		...
		for (int i = 0; i < manageIds.length; i++) {
			map.put("manageId",manageIds[i]);
			dao.update("SQWJ02.updateManageByParentsId", map);
		}
	...
	public EiInfo standardName(EiInfo inInfo) {
		//获取账套
		String datagroupcode = com.baosight.wilp.utils.DatagroupUtil.getDatagroupCode();
		Map map = new HashMap();
		//封装数据
		map.put("datagroupcode", datagroupcode);
		//查询数据
		List<SQBZ01> list = dao.query("SQWJ01.queryStandardName", map);
		//构建返回信息
