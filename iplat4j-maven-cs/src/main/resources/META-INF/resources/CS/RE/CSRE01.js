		...
		"result" : {
			...
			loadComplete : function(grid) {
				...
				$("#REGISTER").on("click", function(e) {
					var url = IPLATUI.CONTEXT_PATH + "/web/CSRE0101";
					var popData = $("#popData");
				...
				$("#EDITREGD").on(
						...
						function(e) {
							...
							if (checkRows.length > 0) {
								...
								NotificationUtil("请选择想要编辑的工单", "error")
							}

						});

				// 撤销按钮
				$("#BACKOUT").on("click", function(e) {
					console.log("撤销");
					// 获取选中的行数据.
					var checkRows = resultGrid.getCheckedRows();
					// 判断是否有选中行.
					if (checkRows.length > 0) {
						// 当前端数据缓存的状态码为01登记状态的时候,可执行撤销操作.
						if (checkRows[0]["statusCode"] == "01") {
							// 将行中taskId工单ID传到后台进行处理.
							var taskId = checkRows[0]["taskId"];
							var backOutCode = '98';
							IPLAT.confirm({
							    message: '<b>将会撤销该登记工单！</br><font color="red">是否确定？</font></b>',
							    okFn: function (e) {
							        //提交数据
							        var eiInfo = new EiInfo();
							        eiInfo.set("taskId", taskId);
							        eiInfo.set("statusCode", backOutCode);
							        EiCommunicator.send("CSRE01", "backout", eiInfo, {
...
//							                }
						...
						NotificationUtil("请选择想要撤销的工单", "error")
					}
				});
