	...
	IPLATUI.EFGrid = {
		"result" : {
			...
			loadComplete : function(grid) {
				// 工单评价按钮
				$("#EVALUATEDONE").on("click", function(e) {
					// 获取所有选中的行数据.
					var checkRows = resultGrid.getCheckedRows();
					// 判断是否有选中行.
					if (checkRows.length > 0) {
						// 当前端数据缓存的状态码为03待评价的时候,可执行评价操作.
						if (checkRows[0]["statusCode"] == "03") {
							...
							NotificationUtil("请选择待评价状态的工单", "error");
						}
					} else {
						NotificationUtil("请选择想要评价的工单", "error")
					}
				});
...
function popDatashow(taskId) {
	var url = IPLATUI.CONTEXT_PATH + "/web/CSPJ0102?initLoad&taskId=" + taskId;
