$(function () {
    ...
    IPLATUI.EFTree = {
        "menu": {
            ...
            loadComplete: function (options) {
                ...
                $("#handleMenu").kendoContextMenu({
                    ...
                    select: function (e) {
                        ...
                        // 当前操作结点的父结点拥有的子结点个数
                        var pNodeChildrenCount = _tree.parent(
                            e.target).find('.k-item').length;
                        /**
                         * 根据右击菜单选项类型进行节点操作
                         * add：新增，delete：删除，edit：编辑
                         */
                        var selectedType = $(e.item).data(
                            "type");
                        // 新增
                        if (selectedType === "add") {
                            ...
                            $("#addNode").unbind('click').click(function () {
                                ...
                                if (validator.validate()) {
                                    ...
                                    if (classifyName == null
                                        || classifyName == "") {
                                        NotificationUtil("保洁分类名称不得为空", "error");
                                        return;
                                    }
                                    ...
                                    EiCommunicator.send("CSSX01", "insert", eiInfo,
                                        {
                                            onSuccess: function () {
                        ...
                        if (selectedType === "delete") {
                            ...
                            WindowUtil({
                                ...
                                ok: function () {
                                    ...
                                    eiInfo.set("parentId",
                                        parentId);
                                    EiCommunicator.send("CSSX01", "deleteBatch", eiInfo, {
                                        onSuccess: function (
                                            eiInfo) {
                                    ...
                                    // setTimeout(function(){window.parent.location.reload()},1);
                                ...
                                function () {
                                    ...
                                    if (validator.validate()) {
                                        ...
                                        if (EclassifyName == null || EclassifyName == "") {
                                            NotificationUtil("保洁事项分类名称不得为空", "error");
                                            return;
                                        }
                                        model2Ei.set("classifyName", $("#EclassifyName").val());
                                        model2Ei.set("memo", $("#Ememo").val());
                                        model2Ei.set("id", id);
                                        EiCommunicator.send("CSSX01", "update", model2Ei, {
                                            onSuccess: function () {
                                                // 重新加载父结点信息
                                                _tree.reload(model[NODE.parentIdField]);
            ...
            /**
             * expand(e)：结点展开前回调函数, e.node 指被展开的结点; collapse(e)：结点收起前回调函数,
             * e.node 指被收起的结点; saveExpanded()：结点展开和收起时，记录结点展开状态
             */
            expand: function () {
                saveExpanded()
            },
        ...
        "result": {
            loadComplete: function (grid) {
                $("#ADD").on(
                    ...
                    function (e) {
                        ...
                        if (moduleId == null || moduleId == "") {
                            NotificationUtil("请选择事项分类添加保洁事项,分类信息节点不能新增事项",
                                "error");
                            return;
                        }
                        var url = IPLATUI.CONTEXT_PATH
                            + "/web/CSSX0101?moduleId=" + moduleId;
                        var popData = $("#popData");
                ...
                $("#EDIT").on("click", function (e) {
                    ...
                    var itemId = checkRows[0].itemId;
                    var url = IPLATUI.CONTEXT_PATH + "/web/CSSX0102?itemId=" + itemId;
                ...
                $("#DELETEROW")
                    .on(
                        ...
                        function (e) {
                            ...
                            IPLAT
                                .confirm({
                                    message: '<b>删除的数据无法恢复！</br><font color="red">是否确定？</font></b>',
                                    okFn: function (e) {
                                        var info = new EiInfo();
                                        info.set("list", arr);
                                        EiCommunicator.send("CSSX01","deleteItem",info,
                                                {
