	...
	public static EiInfo getSpotAllPage(Object param,String resultId) {
		...
		if(staff != null){
			if(StringUtils.isBlank(resultId) ){
				...
				block.addRows((List) staff.getBlock("result").getRows());
//				block.addRows((List) staff.getAttr().get("result"));
	 ...
	 * @throws
	 */
	public static boolean pushWxMsg(List<String> workNoList, List<String> paramList, String templateCode,
								  String appCode){
		EiInfo inInfo = new EiInfo();
		inInfo.set("workNoList", workNoList);
		inInfo.set("paramList", paramList);
		inInfo.set("templateCode", templateCode);
		inInfo.set("appCode", appCode);
		EiInfo info = invoke(inInfo,"S_MC_FW_05");
		if(info == null){
			return false;
		} else {
			return true;
