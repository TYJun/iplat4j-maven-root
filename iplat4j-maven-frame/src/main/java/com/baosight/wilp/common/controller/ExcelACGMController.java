...
@WebServlet("/excelACGM")
@MultipartConfig
public class ExcelACGMController extends HttpServlet {

	// 物资表头
	String[] mat = { "物资名称", "物资分类编码", "物资规格", "物资型号", "计量单位编码", "最近订购单价", "供应商", "制造商", "物资大类编码", "备注" };
	String[] matFieldName = { "matName", "matClassId", "matSpec", "matModel", "unit", "price", "supplier",
			"manufacturer", "matTypeCode", "remark", "error" };

	/**
	 * 模板下载 入参：HttpServletRequest，HttpServletResponse 出参：void
	 * 作者：jzm
	 * 处理：
	 * 1.自动生成模板
	 * 2.设置返回消息头
	...
	/**
	 * 模板上传，导入数据 入参：HttpServletRequest,HttpServletResponse 出参：void 处理：
	...
	public static EiInfo openExcelByInputStream(InputStream inputStream) {
		try {
			...
			if ("物资".equals(sheetName)) {
				Sheet sheet = wb.getSheetAt(0);
				for (int i = 1; i < sheet.getPhysicalNumberOfRows(); i++) {
					Row row = sheet.getRow(i);// 获取行
					Map<String, String> map = new HashMap<>();
					map.put("matName", getValue(row.getCell(0)));
					map.put("matClassId", getValue(row.getCell(1)));
					map.put("matSpec", getValue(row.getCell(2)));
					map.put("matModel", getValue(row.getCell(3)));
					map.put("unit", getValue(row.getCell(4)));
					map.put("price", getValue(row.getCell(5)));
					map.put("supplier", getValue(row.getCell(6)));
					map.put("manufacturer", getValue(row.getCell(7)));
					map.put("matTypeCode", getValue(row.getCell(8)));
					map.put("remark", getValue(row.getCell(9)));
					list.add(map);
				}
				HashMap<String, Object> map1 = new HashMap<>();
				map1.put("list", list);
				eiInfo.setAttr(map1);
				eiInfo.set(EiConstant.serviceName, "ACGM01");
				eiInfo.set(EiConstant.methodName, "importMatFromExcel");
