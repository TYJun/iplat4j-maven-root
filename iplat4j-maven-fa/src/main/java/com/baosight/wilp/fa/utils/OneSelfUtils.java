...
public class OneSelfUtils {
    ...
    public synchronized static String privateCreateCode(String typeCode) {
        ...
        if (CollectionUtils.isNotEmpty(collect)) {
            ...
            List<String> goodNumList = dao.query("FADA01.queryMaxGoodNum", new HashMap<String,String>(){{
                put("name", finalName);
                put("length", finalLength);
            }});
...
//			return BaseDockingUtils.pushWxMsg(workNoList, paramList,"TP00001", appCode);
     ...
     * 新的固定资产权限判断逻辑
     * 资产科：拥有所有权限和所有科室的权限
     * 资产科下属:拥有除报废审批，鉴定，归口权限查看所有科室
     * 盘点员：拥有对应科室的报废资产选择鉴定科室的权限，查看对应挂了盘点的权限
     * 科室负责人：拥有调拨，报废，报废的审批权限。
     * 护长：拥有报废的权限，查看对应科室
     * 科室资产管理员：拥有调拨，报废的权限，查看对应科室
     * 鉴定科室：报废资产的鉴定权限
     * 归口科室：报废鉴定资产的审批权限
     * 通过传入的工号返回用户权限和可以查看的科室
     */
    public static Map<String, Object> queryRolesByWorkNo(String workNo) {
        Map<String, Object> map = new HashMap<>();
        // 查询对应工号的权限
        List<Map<String, String>> roleList = dao.query("FASQ01.queryRolesByWorkNo", workNo);
        if (CollectionUtils.isNotEmpty(roleList)) {
            Map<String, String> roleMap = roleList.get(0);
            String roleStr = roleMap.get("permissionsCode");
            String lookDeptNameStr = roleMap.get("lookDeptName");
            if (StringUtils.isNotEmpty(roleStr)) {
                map.put("role", roleStr);
                // 如果权限包括资产科/资产科下属，可以查看所有的科室
                if (roleStr.contains("FA_LEADER") || roleStr.contains("FA_MEMBER")) {
                    map.put("lookDeptName", null);
                } else {
                    // 如果其他权限，可以查看到包含科室
                    String[] split = lookDeptNameStr.split(",");
                    List<String> list = Arrays.asList(split);
                    map.put("lookDeptName", list);
                }
            }
        }
        return map;
     ...
     * 其他人只能查到拥有资产的科室
     */
    public static List<Map<String, String>> queryDeptsByWorkNo(String workNo) {
        ...
        if (CollectionUtils.isNotEmpty(roleList)) {
            ...
            String roleStr = roleMap.get("permissionsCode");
            String lookDeptNameStr = roleMap.get("lookDeptName");
            if (StringUtils.isNotEmpty(roleStr) && StringUtils.isNotEmpty(lookDeptNameStr)) {
                // 如果权限包括资产科/资产科下属，可以查看所有的科室
                if (roleStr.contains("FA_LEADER") || roleStr.contains("FA_MEMBER")) {
                    list = dao.query("FADA01.queryDept", new HashMap<>());
                } else {
                    String[] split = lookDeptNameStr.split(",");
