...
public class CompareUtils {
    ...
    private static void setResult(Object o1, Object o2, String name, String label, List<ComparisonResult> list, String id, String batchId) {
        ...
        result.setPrevious(r1);
        result.setLater(r2);
        if ("buyCost".equals(name) || "netAssetValue".equals(name) || "totalDepreciation".equals(name) || "equityFund".equals(name) ||
                "otherFund".equals(name)) {
            BigDecimal previous = new BigDecimal(r1);
            BigDecimal later = new BigDecimal(r2);
            BigDecimal subtract = later.subtract(previous).setScale(2, BigDecimal.ROUND_HALF_UP);
            result.setChangeValue(subtract.toString());
        }
