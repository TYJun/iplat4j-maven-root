package com.baosight.wilp.pm.db.service;
...
public class ServicePM0301 extends ServiceBase {
	...
	public EiInfo queryVia(EiInfo inInfo){
		...
		//数据返回
		return PMUtils.BuildOutEiInfo(inInfo, "resultA", new TpmVia().eiMetadata, list, count);
	}
	...
	public EiInfo viaProject(EiInfo inInfo){
		...
		if(viaSmsMap.isEmpty()){
			List<Map<String,String>> queryList = dao.query("PM0301.querySendVia",null);
			queryList.forEach(map -> viaSmsMap.put(map.get("projectPk"), map.get("workNos")));
		}
		//保存督办信息
