    ...
    $(function () {
        ...
        $("#EXCUTEPR").on("click", function (e) {
            ...
            if (checkRows.length > 0) {
                ...
                IPLAT.NotificationUtil("请选择需要执行的项目", "error");
                return;
            }
        });
        ...
        $("#EDITPR").on("click", function (e) {
            var checkRows = resultGrid.getCheckedRows();
            if (checkRows.length > 0) {
                if (checkRows[0].projectStatus == "02") {
                    projectEditWindowOpen(checkRows[0], "excute");
                } else {
                    IPLAT.NotificationUtil("请选择执行状态的项目", "error");
                }
            } else {
                IPLAT.NotificationUtil("请选择需要编辑的项目", "error");
                return;
            }
        });
        ...
        // 作废
        $("#CANCELLPR").on("click", function () {
            var checkRows = resultGrid.getCheckedRows();
            var eiInfo = new EiInfo();
            eiInfo.set("projectNo", checkRows[0].projectNo);
            if (checkRows.length == 0) {
                IPLAT.NotificationUtil("请选择需要作废的数据", "warning");
                return;
            }
            IPLAT.confirm({
                message: '<b>确定执行该操作吗？</b></i>',
                okFn: function (e) {
                    EiCommunicator.send("PM01", "cancell", eiInfo, {
                        onSuccess: function (ei) {
                            IPLAT.NotificationUtil(ei.msg)
                            resultGrid.dataSource.page(1);
                        }
