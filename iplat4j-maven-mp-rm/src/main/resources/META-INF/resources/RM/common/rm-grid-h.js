class WilpGrid {
    constructor(config) {
        ...
        this.resultId = config.resultId ?? "result";
        /**是否有操作类型参数,一般拼接到url后面(如: xx?xx=xx&type=add)**/
        this.isOpType = config.isOpType ?? true;
        /**是否详情功能,要求Grid的某列有displayType="url"**/
        this.detail = config.detail ?? true;
        /**Grid详情页面地址**/
        this.detailUrl = config.detailUrl ?? __ei.serviceName+"01?inqu_status-0-id=#:id#";
        this.detailWindow = config.detailWindow ?? "popData";
        /**是否有新增按钮(button的id为ADD)**/
        this.add = config.add ?? true;
        /**是否复制新增**/
        this.addCopy = config.addCopy ?? false;
        /**新增页面地址**/
        this.addUrl = config.addUrl ?? __ei.serviceName+"01?inqu_status-0-id=#:id#";
        /**是否有编辑按钮(button的id为EDIT)**/
        this.edit = config.edit ?? true;
        /**编辑页面地址**/
        this.editUrl = config.editUrl ?? __ei.serviceName+"01?inqu_status-0-id=#:id#";
        /**编辑配置**/
        this.editConfig = config.editConfig ?? {status: ["01","20"]}
        /**是否有删除按钮(button的id为DEL)**/
        this.del =  config.del ?? true;
        /**
         * 删除配置：
         *    status: 数据可删除状态
         *    serviceName: 服务名
         *    methodName: 方法名
         **/
        this.delConfig = config.delConfig ?? {status: ["01"], serviceName: __ei.serviceName, methodName: "delete"};
        /**是否有提交按钮(button的id为SUBMIT)**/
        this.submit = config.submit ?? false;
        /**
         * 提交配置
         *    status: 数据可删除状态
         *    serviceName: 服务名
         *    methodName: 方法名
         **/
        this.submitConfig = config.submitConfig ?? { status: ["01"], backStatus: ["20"], serviceName: __ei.serviceName, methodName: "submit" };
         ...
         *    methodName: 方法名
         **/
        this.backConfig = config.backConfig ?? {status: ["10"], serviceName: __ei.serviceName, methodName: "withdraw"};
        /**是否展示Grid的分页**/
        this.showPage = config.showPage ?? true;
        /**Grid的自定义按钮配置**/
        ...
        /**是否有行点击事件**/
        this.hasRowClick = config.hasRowClick ?? false;
        /**行点击事件配置
         *      gridId : 子表格ID
         *      paramName : 参数名称
         *      colName : 列表字段名称
         **/
        this.rowClickConfig = config.rowClickConfig ?? {};
        /**是否显示图片**/
        this.showImg = config.showImg ?? false;
        /**是否有单元格编辑事件**/
        this.hasAfterEditor = config.hasAfterEditor ?? false;
        /**
         * 单元格编辑后事件配置
         *      isShow: 是否存在
         *      call: 方法
         */
        this.afterEditConfig = config.afterEditConfig ?? {isShow: false}
    }
     ...
     */
    buildUrl(url, data, isOpType, type) {
        let reg = /([^?&=]+)=([^&?=]+)/ig, params = [];
