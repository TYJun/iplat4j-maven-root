...
public class RmUtils {
     ...
     * @throws
     **/
    public static List<String> getUserDept(String workNo) {
        EiInfo eiInfo = RmUtils.invoke("RMTY01", "queryUserBusinessDept", Arrays.asList("workNo"), workNo);
        List<Map<String, String>> list = eiInfo.getBlock("userDept").getRows();
        if(CollectionUtils.isEmpty(list)) {
            return new ArrayList<>();
        } else {
            return list.stream().map(map -> map.get("deptNum")).collect(Collectors.toList());
        }
    }
