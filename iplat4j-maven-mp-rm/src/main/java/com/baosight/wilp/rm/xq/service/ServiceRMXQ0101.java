...
public class ServiceRMXQ0101 extends ServiceBase {
    ...
    public EiInfo initLoad(EiInfo inInfo) {
        ...
        if (!RmConstant.OPERATE_TYPE_ADD.equals(inInfo.getString(RmConstant.OPERATE_NAME))) {
            ...
            UserSession.setOutRequestProperty("planType", list.get(0).getPlanType());
        } else {
            Map<String, Object> deptMap = BaseDockingUtils.getDeptByworkNo(UserSession.getLoginName());
            inInfo.setCell(RmConstant.QUERY_BLOCK,0, "planTime", String.valueOf(LocalDate.now().plusYears(1).getYear()));
            inInfo.setCell(RmConstant.QUERY_BLOCK,0, "manageDeptNum", RmUtils.toString(deptMap.get("deptNum")));
            inInfo.setCell(RmConstant.QUERY_BLOCK,0, "manageDeptName", RmUtils.toString(deptMap.get("deptName")));
            //复制新增
            ...
            if(StringUtils.isNotBlank(id)) {
                ...
                inInfo.setRows(RmConstant.RESULT_BLOCK, initLoadData());
            }
        }
        //像request域中添加参数
     ...
     * @throws
     **/
    private List<Map> initLoadData() {
        Map<String, Object> pMap = new HashMap<>(2);
        List<String> userDept = RmUtils.getUserDept(UserSession.getLoginName());
        pMap.put("costDepts", userDept);
        //处理时间
        if(LocalDate.now().getYear() == 2023){
            pMap.put("beginTime", "2023-04-01");
            pMap.put("endTime", "2023-06-30");
        } else {
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            LocalDate lastMonthDate = LocalDate.now().minusMonths(1);
            pMap.put("endTime",lastMonthDate.withDayOfMonth(lastMonthDate.lengthOfMonth()).format(formatter));
            pMap.put("endTime",lastMonthDate.minusYears(1).plusMonths(1).withDayOfMonth(1).format(formatter));
        }
        List<Map> list = dao.query("RMLJ01.queryHistoryClaim", pMap);
        list.forEach(map -> map.put("referNum", NumberUtils.toDouble(map.get("historyNum"),0d)*4));
        return list;
    }
