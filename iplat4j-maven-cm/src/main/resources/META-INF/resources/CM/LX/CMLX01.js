/* 获取选中的全局变量 */
		...
		"result" : {
			...
			loadComplete: function (e) {
				...
				$("#ADD").on("click",function(e){
					...
					popDataWindow.setOptions({"title":"新增合同类型定义"}); 
					popData("","add");
				});
				...
				$("#LOOK").on("click",function(e){
					if (datagrid == null) {
						...
						popDataWindow.setOptions({"title":"查看合同类型定义"}); 
				...
				$("#EDIT").on("click", function() {
					if (datagrid == null) {
						...
						popDataWindow.setOptions({"title":"编辑合同类型定义"}); 
	...
	//弹窗配置
 	function popData(id,type) {
		var url = IPLATUI.CONTEXT_PATH + "/web/CMLX0101?initLoad&id="+id+"&type="+type;
		var popData = $("#popData");
